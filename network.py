...
def mse_loss(y_true, y_pred):
    # y_true and y_pred and numpy arrays of the same length
...
#Data shifted back by mean value
data = np.array(
    [
        [-0.2, 0.0],  
        [0.8,  1.0],
...
jane = np.array([2.8, 1]) # 8, 8
tim = np.array([-2.2, -4])  # 3, 3
print("Jane: %.3f" % network.feedforward(jane)) # 0.969
print("Tim: %.3f" % network.feedforward(tim)) # 0.015